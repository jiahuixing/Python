...
class Bugreport:
    ...
    def get_bugreport(self):
        ...
        if len(self.adb_device_list) > 0:
            for i in xrange(len(self.adb_device_list)):
                ...
                self.txt_name = '%s%s' % (self.file_name, Info.bug_suffix)
    ...
    def tar_file(self):
        ...
        self.tgz_name = '%s%s' % (self.file_name, Info.tgz_suffix)
        cmd = 'tar -zcf %s %s%s' % (self.tgz_name, self.file_name, Info.bug_suffix)
        color_cmd = 'tar -zcf %s %s%s' % (color_msg(self.tgz_name), self.file_name, Info.bug_suffix)
        debug_msg(color_cmd)
        os.system(cmd)

    def mv_tgz_txt(self):
        debug_msg(color_msg('------push_tgz------', RED))
        self.mv_file(self.tgz_name, Info.svn_path)
        self.mv_file(self.txt_name, Info.svn_path)
